...
data_source = '/proj/cse349-449/djd225/final/exports_11/*/*.xml'
...
def build_pipeline(use_lemmatizer=False, use_stemmer=False, lowercase=True, remove_stopwords=True, use_ngrams=None, cleanup_mode="shrink"):
    ...
    ngram_outputs = []
    if use_ngrams:
        for n in (use_ngrams if isinstance(use_ngrams, list) else [use_ngrams]):
            ngram_generator = NGramGenerator() \
                .setN(n) \
                .setInputCols([final_output]) \
                .setOutputCol(f"ngrams_{n}")
            stages.append(ngram_generator)
            ngram_outputs.append(f"ngrams_{n}")
        final_output = ngram_outputs
    else:
        final_output = [final_output]

    output_cols = [f"tokens_{col}" for col in range(len(final_output))]
...
pipeline_variants = [
    {"use_lemmatizer": False, "use_stemmer": False, "lowercase": True, "remove_stopwords": True, "use_ngrams": [1, 2], "cleanup_mode": "shrink"},
    {"use_lemmatizer": False, "use_stemmer": False, "lowercase": False, "remove_stopwords": True, "use_ngrams": [1, 2], "cleanup_mode": "shrink"},
    {"use_lemmatizer": False, "use_stemmer": False, "lowercase": True, "remove_stopwords": True, "use_ngrams": [1, 2], "cleanup_mode": "disabled"},
    {"use_lemmatizer": False, "use_stemmer": False, "lowercase": True, "remove_stopwords": True, "use_ngrams": 2, "cleanup_mode": "shrink"},
    {"use_lemmatizer": False, "use_stemmer": False, "lowercase": True, "remove_stopwords": True, "use_ngrams": [1, 2, 3], "cleanup_mode": "shrink"},
    {"use_lemmatizer": False, "use_stemmer": False, "lowercase": False, "remove_stopwords": True, "use_ngrams": 2, "cleanup_mode": "shrink"},
]
...
with open(f"{output_dir}/output.txt", 'w', encoding='utf-8') as output_file:
    for i, params in enumerate(pipeline_variants, start=1):
        ...
        # Dynamically handle token column(s)
        if isinstance(params.get("use_ngrams"), list):
            token_cols = [f"tokens_{i}" for i in range(len(params["use_ngrams"]))]
        elif params.get("use_ngrams"):
            token_cols = ["tokens_0"]
        else:
            token_cols = ["tokens"]

        # Combine token columns into one array if multiple are present
        if len(token_cols) > 1:
            tokens_df = processed_df.withColumn("tokens", F.array_distinct(F.concat(*[F.col(c) for c in token_cols])))
        else:
            tokens_df = processed_df.withColumnRenamed(token_cols[0], "tokens")

        # Prepare data for vectorization
        tokens_df = tokens_df.select("title", "year", "tokens")
